...
for(var i=0;i<3;i++){
	...
	var matriz_Y=new THREE.Matrix4();
	var matriz_Z=new THREE.Matrix4();

	var angulo= Math.PI/2;
	matriz_X.makeRotationX(angulo);
	matriz_Y.makeRotationY(angulo);
	matriz_Z.makeRotationZ(angulo);

	Cubo[0].translateX(delta);
	Cubo[0].position.applyMatrix4(matriz_Z);
	Cubo[1].translateY(delta);
	Cubo[1].position.applyMatrix4(matriz_X);
